...
class SecurityContractTests(unittest.TestCase):
    ...
    def test_live_health_menu_uses_non_seekable_tty_safely(self):
        ...
        self.assertIn("proxyfleet_python live-select", text)
        self.assertIn("--selection-output", text)
        self.assertIn("</dev/tty >/dev/tty", text)
    ...
    def test_live_health_menu_updates_lines_without_fullscreen_redraw(self):
        text = (ROOT / "scripts" / "proxyfleet-master.sh").read_text(encoding="utf-8")
        live_select = (ROOT / "src" / "proxyfleet" / "live_select.py").read_text(encoding="utf-8")
        self.assertIn("curses.wrapper", live_select)
        self.assertIn("viewport_height", live_select)
        self.assertIn("search_mode", live_select)
        self.assertIn("KEY_DOWN", live_select)
        self.assertIn("KEY_UP", live_select)
        self.assertIn("sort_by_delay", live_select)
        self.assertIn("MihomoClient", live_select)
        self.assertNotIn("select_node(", live_select)
        self.assertNotIn("write_desired_state", live_select)
        self.assertNotIn("run_salt_sync", live_select)
        self.assertNotIn(".select_node(", live_select)
