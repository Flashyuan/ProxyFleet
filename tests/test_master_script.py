...
class MasterScriptTuiTests(unittest.TestCase):
    ...
    def test_select_sync_resource_optimization_flags(self):
        ...
        self.assertIn('effective_salt_root="${plan_salt_root}"', text)
        self.assertIn("publish_args[3]=\"${effective_salt_root}\"", text)
        self.assertIn('[[ "${full_converge}" != "true" && "${plan_only}" != "true" ]]', text)
        self.assertIn("请确认后显式执行：select-sync --full-converge", text)
        self.assertIn('"${publish_output}" == *"E_SYNC_NEEDS_FULL_CONVERGE"*', text)
        self.assertNotIn("自动执行一次 full-converge", text)
        self.assertNotIn('[[ "${arg}" == "--lightweight" ]] && continue', text)
