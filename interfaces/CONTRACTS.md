...
driver: native-mihomo | shellcrash-adopted | shellcrash-compat
...
拥有 `/etc/mihomo`、`mihomo.service` 和 API secret。

### 8.2 `shellcrash-adopted`

不拥有 ShellCrash 透明代理生命周期，但拥有最终受管 release；所有写入必须经过已验证持久化入口。

### 8.3 `shellcrash-compat`

只允许 Provider/策略组选择的有限操作；不承诺 config hash 一致。
