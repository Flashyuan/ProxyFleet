...
`component-locks.json` 是 Mihomo 二进制版本来源。当前锁定候选为：
...
4. 缺少二进制 SHA-256 锁定值时，不得安装或升级 Mihomo 二进制，
...
安装或升级二进制前必须具备组件锁中的 SHA-256。若 `sha256` 为 `null`，
...
```text
/etc/mihomo/
  current -> /etc/mihomo/releases/<release_revision>
  previous -> /etc/mihomo/releases/<previous_release_revision>
  releases/
  api-secret
mihomo.service
```
